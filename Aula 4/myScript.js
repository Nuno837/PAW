/* 
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */

var btna = document.getElementById('a');
var btnb = document.getElementById('b');
var btnc = document.getElementById('c');
var main = document.querySelector('p');

btna.addEventListener('click', updateBtnA);
btnb.addEventListener('click', updateBtnB);
btnc.addEventListener('click', updateBtnC);

function updateBtnA() {
  if (btna.textContent === 'a') {
    document.getElementById('main').src="http://creativeitsoft.net/techlogo/css3.png";
  } else {
    main.textContent = 'ERROR';
  }
}

function updateBtnB() {
  if (btnb.textContent === 'b') {
    document.getElementById('main').src="data:image/png;base64,";
  } else {
    main.textContent = 'ERROR';
  }
}

function updateBtnC() {
  if (btnc.textContent === 'c') {
    document.getElementById('main').src="http://luizricardo.org/wordpress/wp-content/upload-files/2014/11/js-logo-badge-512.png";
  } else {
    main.textContent = 'ERROR';
  }
}
